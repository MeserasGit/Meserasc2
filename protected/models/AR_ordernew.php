...
{	
	...
	public function rules()
	{
		 return array(
			...
			admin_base_currency,exchange_rate_merchant_to_admin,exchange_rate_admin_to_merchant,exchange_rate_use_currency_to_admin,on_demand_availability
			','safe'),
				...
				if($model->payment_code=="cod"){
					if($model->attr1==1){				
						if($this->payment_change<=0){
							...
							if($this->total>$this->payment_change){					
	...
	{									
		...
		if ( $this->scenario=="new_order"){
			...
			//dump(CommonUtility::getHomebaseUrl()."/task/debitdiscount?".http_build_query($get_params) );die();
