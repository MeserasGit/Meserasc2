...
if(in_array('distance',(array)$payload) && $transaction_type=="delivery" ){	
	try {
		$distance_resp = CCart::getDistance( Yii::app()->user->id,$local_id,$unit,$merchant_lat,$merchant_lng);
		$distance = floatval($distance_resp['distance']);
		$address_component = $distance_resp['address_component'];
		if($distance_covered>0 && $distance>0){
			if($distance>$distance_covered){
...
if(in_array('service_fee',(array)$payload)){	
	if($service_charge){		
		...
		if($servicefee>0){
			$service_fee = $charge_type=="percentage"? (($servicefee/100) * $temp_sub_total) : ($servicefee*$exchange_rate) ;			
...
if(in_array('card_fee',(array)$payload)){
	try {		
		...
		if($payment_method = CPayments::defaultPaymentByMerchant(Yii::app()->user->id, $merchant_type )){
   	  ...
   	  foreach ($cart_condition as $condition) {
			  ...
			  if($meta_value['type']=="voucher"){							
				try {
					...
					$promo_details = CPromos::applyVoucher($merchant_id, $meta_value['id'] , Yii::app()->user->id , $now , $sub_total,$transaction_type);					
					//$meta_value['value'] = - ($promo_details['less_amount']*$exchange_rate);					
