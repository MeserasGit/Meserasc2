...
class CMerchantListingV1
{
	...
	public static function getMerchantInfo($slug_name='',$lang='')
	{
		...
		a.address,
		a.restaurant_slug, a.latitude,a.lontitude,a.distance_unit,
		a.contact_phone,
		b.review_count,
		b.ratings,
		...
		if($res = CCacheData::queryAll($stmt)){									
			foreach ($res as $val) {
				...
				if(isset($val2['distance'])){
				    ...
				    $val2['distance_pretty'] = "$distance $unit";
				} else {
					$val2['distance'] = '';
		...
		* cos( radians( lontitude ) - radians($filter[lng]) ) 
		...
		if($model){
			...
			foreach ($model as $items) {						
				...
				$distance = Price_Formatter::convertToRaw($items->distance,1);
				$data[] = [
		...
		if($model){
			...
			foreach ($model as $items) {				
				$data[$items->merchant_id] = [
					...
					'rating'=>intval($items->ratings),
					'ratings'=>number_format(intval($items->ratings),1,'.',''),
				];
			}
			return $data;
		}
