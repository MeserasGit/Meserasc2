...
{	
					    ...
					    case "notification_to_driver":
							try {
								...
								if($driver_data->notification==1){
									...
									if(!empty($sms_template)){
										$resp = CommonUtility::sendSMS($driver_phone,$sms_template,$driver_data->driver_id,0,$driver_data->first_name);
									}    						
	...
	private function processTemplate($template_id=0, $template_type= array(), $data=array(), $to=array())
	{
		...
		foreach ($templates as $items) {
			...
			if($items['template_type']=="email" && $items['enabled_email']==1 ){
				...
				$resp = CommonUtility::sendSMS( isset($to['mobile_phone'])?$to['mobile_phone']:'' ,$sms_template);
	...
	public function actionorderautoaccept()
	{
		try {
			...
			dump($criteria);
			if($model = AR_ordernew::model()->findAll($criteria)){				
				$template_id = CNotifications::getStatusActionSingle($status,'notification_to_customer');
				dump("template_id=>$template_id");
				foreach ($model as $items) {					
					dump($items->order_id);
					$items->status = $status;
					$items->save();
					try {																
						$this->runOrderSingleAction($items->order_uuid,$template_id);
					} catch (Exception $e) {		    	
						$this->msg[] = t($e->getMessage());
					}	
				}
